Attribute VB_Name = "10_modAnalisisAux"

Option Compare Database
Option Explicit

'===============================================================
' Módulo: 10_modAnalisisAux
...
Public Function AnalizarProyecto() As clsCatalogoInspector
    ...
    If Not VerificaVBIDEinterno() Then Exit Function
    ...
    For Each vbComp In Application.VBE.ActiveVBProject.VBComponents
        ...
        Select Case Tipo
            Case vbext_ct_StdModule
                cat.AgregarModulo AnalizarModulo(vbComp, cat)

            Case vbext_ct_ClassModule
                cat.AgregarClase AnalizarClase(vbComp, cat)

            Case vbext_ct_MSForm
                cat.AgregarUserForm AnalizarModulo(vbComp, cat)

            Case vbext_ct_Document
                Select Case TipoModuloObjeto(vbComp)
                    Case "Formulario": cat.AgregarFormulario AnalizarModulo(vbComp, cat)
                    Case "Informe":    cat.AgregarInforme AnalizarModulo(vbComp, cat)
                    Case Else:         cat.AgregarOtro AnalizarModulo(vbComp, cat)
                End Select
        End Select
    Next vbComp

    BuscarReferenciasEnProyecto cat
    cat.CatalogoSimbolos.ActualizarEstadisticas
...
'---------------------------------------------------------------
Public Function AnalizarModulo(vbComp As VBIDE.VBComponent, _
                               cat As clsCatalogoInspector) As clsModulo
    ...
    ' Declaraciones de módulo ? ahora recibe cat
    DetectarDeclaraciones code, m.nombre, "", cat

    m.CalcularMetricasModulo

    ' Detectar miembros ? ahora recibe cat
    Set m.Miembros = DetectarMiembros(code, m.nombre, cat)
...
' Analiza una clase
'---------------------------------------------------------------
Public Function AnalizarClase(vbComp As VBIDE.VBComponent, _
                              cat As clsCatalogoInspector) As clsClase
    ...
    ' Declaraciones de clase ? ahora recibe cat
    DetectarDeclaraciones code, c.nombre, "", cat

    c.CalcularMetricasClase

    ' Detectar miembros ? ahora recibe cat
    Set c.Miembros = DetectarMiembros(code, c.nombre, cat)
...
Public Function DetectarMiembros(code As VBIDE.CodeModule, _
                                 ...
                                 cat As clsCatalogoInspector) As Collection
    ...
    For i = 1 To code.CountOfLines
        ...
        If EsInicioMiembro(linea) Then
            ...
            ' Declaraciones locales dentro del miembro ? ahora recibe cat
            DetectarDeclaracionesLocales code, m, nombreModulo, cat
