Attribute VB_Name = "14_modReglas"
...
Private Sub Regla_NombresReservados(cat As clsCatalogoInspector, resultados As Collection)
    ...
    For Each sim In gCatalogoSimbolos.Simbolos
        If EsPalabraReservada(sim.nombre) Then
            resultados.Add CrearResultado( _
                sevAviso, teMiembro, sim.modulo, sim.miembro, sim.Tipo, _
                "El nombre '" & sim.nombre & "' coincide con una palabra reservada.", _
                "R-PR-001", "", False, sim.LineaDeclaracion)
        End If
    Next sim
End Sub
