...
Private Sub RepararResultado(res As clsResultadoAnalisis)
    ...
    Select Case True
        ...
        Case res.codigoReparacion = "SUGERIR_RENOMBRE_RESERVADO"
            SugerirRenombreReservado res
...
'---------------------------------------------------------------
' Reparación: sugerir renombre por colisión con palabra reservada
'---------------------------------------------------------------
Private Sub SugerirRenombreReservado(res As clsResultadoAnalisis)
    Dim vbProj As VBIDE.VBProject
    Dim comp As VBIDE.VBComponent
    Dim cm As VBIDE.CodeModule
    Dim comentario As String

    If Len(res.nombreElemento) = 0 Then
        Debug.Print "  >> No se puede sugerir renombre: nombre de módulo vacío."
        Exit Sub
    End If
    ...
    For Each comp In vbProj.VBComponents
        If comp.Name = res.nombreElemento Then
            Set cm = comp.CodeModule

            comentario = "' SUGERENCIA: El nombre '" & res.nombreElemento & _
                         "' coincide con una palabra reservada. Considere renombrarlo."

            On Error GoTo ErrHandler
            cm.InsertLines 1, comentario
            Debug.Print "  >> Comentario de sugerencia añadido en "; res.nombreElemento
            Exit Sub

ErrHandler:
            Debug.Print "  >> Error al insertar sugerencia en "; res.nombreElemento; ": "; Err.Description
            Exit Sub
        End If
    Next comp

    Debug.Print "  >> No se encontró el módulo: "; res.nombreElemento
End Sub
