...
Public Sub MF_MarcarTonicaES( _
...
'        End If
'
'        ' 3. PATRÓN CCV ? V C | C V (ALMA, DOCTOR…)
'        If EsConsonante_ES(c1) And EsConsonante_ES(c2) Then
'            If i < Len(Texto) Then
'                c3 = Mid$(Texto, i + 1, 1)
'                If EsVocal_ES(c3) Then
...
'
''' =====================================================
''' 4. VOCAL + VOCAL ? hiato o diptongo
'''     Extensión: V H V ? tratar como VV si c1–c3 forman diptongo
''' =====================================================
''
''' Caso especial: V H V ? diptongo si c1–c3 lo son
''If c2 = "H" Then
''    If i < Len(Texto) Then
''        c3 = Mid$(Texto, i + 1, 1)
''        If EsVocal_ES(c1) And EsVocal_ES(c3) Then
''            If EsDiptongo_ES(c1, c3) Then
''                ' Se comporta como diptongo AU, EU, OI, etc.
''                GoTo Siguiente
''            End If
''        End If
''    End If
''End If
''
''' Caso normal: VV contiguo
''If EsVocal_ES(c1) And EsVocal_ES(c2) Then
''    If Not EsDiptongo_ES(c1, c2) Then
''        col.Add Array(ini, i - 1)
''        ini = i
''    End If
''End If
'
'        ' 4. PATRÓN VCV ? V | CV, salvo V "H" V con diptongo
'        If EsVocal_ES(c1) And EsConsonante_ES(c2) Then
'            If i < Len(Texto) Then
'                c3 = Mid$(Texto, i + 1, 1)
'                If EsVocal_ES(c3) Then
'                    ' Excepción: V H V donde V–V forman diptongo (AHU, AHEI…)
'                    If Not (c2 = "H" And EsDiptongo_ES(c1, c3)) Then
'                        col.Add Array(ini, i - 1)
'                        ini = i
'                        GoTo Siguiente
'                    End If
...
'        End If
'
'        ' 5. VOCAL + VOCAL ? hiato o diptongo
'        If EsVocal_ES(c1) And EsVocal_ES(c2) Then
'            If Not EsDiptongo_ES(c1, c2) Then
'                col.Add Array(ini, i - 1)
'                ini = i
...
'
'
''        ' 6. CONSONANTE + VOCAL ? posible corte CV
''        ElseIf EsConsonante_ES(c1) And EsVocal_ES(c2) Then
''            If i > 2 Then
''                c0 = Mid$(Texto, i - 2, 1)
''                If EsVocalFuerte_ES(c0) Then
...
'
'' =====================================================
'' 3. PATRÓN CCV ? decidir si se corta C | CV
'' =====================================================
'If EsConsonante_ES(c1) And EsConsonante_ES(c2) Then
'    If i < Len(Texto) Then
'        c3 = Mid$(Texto, i + 1, 1)
...
'
''        ' =====================================================
''        ' 3. H MUDA — No rompe sílabas ni diptongos
''        ' =====================================================
''        ' Si hay H entre vocales, se ignora para la lógica VV
''        If c1 = "H" Or c2 = "H" Then
''            GoTo Siguiente
''        End If
'
'        ' =====================================================
...
'
'        ' =====================================================
'        ' 5. CONSONANTE + VOCAL ? posible corte CV
'        ' =====================================================
'        ElseIf EsConsonante_ES(c1) And EsVocal_ES(c2) Then
'
'            If i > 2 Then
...
'
'    ' =====================================================
'    ' 6. Última sílaba (con eliminación de H final)
'    ' =====================================================
'    If ini <= Len(Texto) Then
'
'        Dim fin As Long
...
'' ============================================================
''   Silabear_ES — Versión optimizada para nombres completos
''   - Respeta espacios
''   - No mezcla sílabas entre palabras
''   - Devuelve posiciones absolutas (ini, fin)
'' ============================================================
...
'        ' =====================================================
'        If c1 = " " Then
'            ' Cerrar sílaba previa si existe
'            If i - 2 >= ini Then
'                col.Add Array(ini, i - 2)
'            End If
'            ini = i
...
'
'        ' =====================================================
'        ' 1. VOCAL + VOCAL ? hiato o diptongo
'        ' =====================================================
'        If EsVocal_ES(c1) And EsVocal_ES(c2) Then
'
'            If Not EsDiptongo_ES(c1, c2) Then
...
'
'        ' =====================================================
'        ' 2. CONSONANTE + VOCAL ? posible corte CV
'        ' =====================================================
'        ElseIf EsConsonante_ES(c1) And EsVocal_ES(c2) Then
'
'            ' Si la vocal anterior es fuerte ? MA-RI-A
'            If i > 2 Then
'                c0 = Mid$(Texto, i - 2, 1)
'                If EsVocalFuerte_ES(c0) Then
'                    col.Add Array(ini, i - 2)
...
'Public Function Silabear_ES1(ByVal Texto As String) As Collection
'
'    Dim col As New Collection
'    Dim i As Long, ini As Long
'    Dim c0 As String, c1 As String, c2 As String, c3 As String
...
'        ' =====================================================
'        ' 4. PATRÓN VCV ? V | CV
'        '     (salvo V-H-V con diptongo)
'        ' =====================================================
'        If EsVocal_ES(c1) And EsConsonante_ES(c2) Then
'            If i < Len(Texto) Then
'                c3 = Mid$(Texto, i + 1, 1)
'                If EsVocal_ES(c3) Then
'                    ' Excepción: V-H-V donde c1–c3 forman diptongo
'                    If Not (c2 = "H" And EsDiptongo_ES(c1, c3)) Then
'                        col.Add Array(ini, i - 1)
'                        ini = i
'                        GoTo Siguiente
...
'' =====================================================
'' 5. VOCAL + VOCAL (VV)
''     Extensión: V-H-V tratado como VV si c1–c3 diptongan
''     EXCEPTO en A-H-U-E / A-H-U-O (a-hue-, a-huo-)
'' =====================================================
'
'' Caso especial: V-H-V ? diptongo si c1–c3 lo son
'If c2 = "H" Then
...
'            If EsDiptongo_ES(c1, c3) Then
'
'                ' Excepción: A-H-U-E / A-H-U-O ? NO diptongo AU
'                If Not (c1 = "A" And c3 = "U" _
'                        And i + 1 < Len(Texto) _
'                        And (Mid$(Texto, i + 2, 1) = "E" Or Mid$(Texto, i + 2, 1) = "O")) Then
'
'                    ' Se comporta como diptongo
'                    GoTo Siguiente
'                End If
'
'            End If
...
'' ================================
'
'' Determina si un carácter es vocal española
'Private Function EsVocal_ES(c As String) As Boolean
'    EsVocal_ES = (InStr("AEIOUÁÉÍÓÚ", c) > 0)
'End Function
'
'' Vocal fuerte (rompe diptongo)
'Private Function EsVocalFuerte_ES(c As String) As Boolean
'    EsVocalFuerte_ES = (InStr("AÁEÉOÓ", c) > 0)
'End Function
'
'' Determina si un carácter es consonante española
'Private Function EsConsonante_ES(c As String) As Boolean
'    EsConsonante_ES = (c <> " " And Not EsVocal_ES(c))
'End Function
'
...
'End Function
'
'' Grupos consonánticos inseparables del español
'Private Function EsGrupoInseparable_ES(par As String) As Boolean
'    Dim g As Variant
'    Dim lista As Variant
'
...
'Private Function EsVocal_ES(c As String) As Boolean
'    EsVocal_ES = (InStr("AEIOUÁÉÍÓÚÜ", c) > 0)
'End Function
'
'Private Function EsVocalFuerte_ES(c As String) As Boolean
'    ' Í y Ú se consideran fuertes (rompen diptongo)
'    EsVocalFuerte_ES = (InStr("AEOÁÉÓÍÚ", c) > 0)
'End Function
'
'Private Function EsConsonante_ES(c As String) As Boolean
'    EsConsonante_ES = (c <> " " And Not EsVocal_ES(c))
...
'
'Private Function EsDiptongo_ES(c1 As String, c2 As String) As Boolean
'    Dim debiles As String, fuertes As String
'    debiles = "IUÜ"
'    fuertes = "AEOÁÉÓ"
'
'    ' Í y Ú NO son débiles ? rompen diptongo
'    If (InStr(fuertes, c1) > 0 And InStr(debiles, c2) > 0) _
'    Or (InStr(debiles, c1) > 0 And InStr(fuertes, c2) > 0) _
'    Or (InStr(debiles, c1) > 0 And InStr(debiles, c2) > 0) Then
'        EsDiptongo_ES = True
'    End If
'End Function
...
'    fuertes = "AEOÁÉÓ"
'
'    ' débil acentuada (Í, Ú) rompe diptongo ? no se incluyen en débiles
'    If (InStr(fuertes, c1) > 0 And InStr(debiles, c2) > 0) _
'    Or (InStr(debiles, c1) > 0 And InStr(fuertes, c2) > 0) _
'    Or (InStr(debiles, c1) > 0 And InStr(debiles, c2) > 0) Then
'        EsDiptongo_ES = True
...
'                    c4 = Mid$(Texto, i + 2, 1)
'                    If EsConsonante_ES(c4) Or c4 = "Y" Then
'                        ' No cortamos aquí: dejamos que AHU se mantenga unido
'                        GoTo Siguiente
'                    End If
'                End If
'            End If
...
'    Dim resultado As New Collection
'    Dim item As Variant
'    Dim s As String, sOriginal As String
'    Dim partes As Variant
'    Dim p As Variant
'    Dim inicio As Long, fin As Long
'    Dim valido As Boolean
...
'    Set col = Silabear_ES(Texto)
'
'    ' 2. Convertir a string con pipes
'    For Each item In col
'        s = s & Mid$(Texto, item(0), item(1) - item(0) + 1) & "|"
'    Next item
'    If Len(s) > 0 Then s = Left$(s, Len(s) - 1)
...
'        msg = ""
'
'        s = InputBox("Revisa o corrige las sílabas:", "Revisión de sílabas", s)
'
'        ' Si el usuario cancela ? devolver silabeo automático
'        If s = "" Then
'            Set Silabear_ES_ConRevision = col
...
'        End If
'
'        ' Validación 3: comprobar que las sílabas reconstruyen exactamente el texto original
'        partes = Split(s, "|")
'        Dim reconstruido As String
'        reconstruido = ""
'
'        For Each p In partes
'            reconstruido = reconstruido & p
'        Next p
'
'        If UCase$(reconstruido) <> UCase$(Texto) Then
'            valido = False
...
'    If Len(s) > 0 Then s = Left$(s, Len(s) - 1)
'
'    ' 3. Mostrar InputBox para revisión manual
'    s = InputBox("Revisa o corrige las sílabas:", "Revisión de sílabas", s)
'
'    ' 4. Reconstruir la colección a partir del string corregido
'    partes = Split(s, "|")
'    inicio = 1
'
'    For Each p In partes
