...
' ============================================================
'   NOTACIÓN FONÉTICA – SISTEMA DE MARCADORES
'   Esta notación se usa para indicar acentos, pausas y
'   modificadores fonémicos dentro de las sílabas.
'
'   1) SEPARADOR SILÁBICO
'      Se usa el carácter:
'          |     (pipe con espacios alrededor)
'
'      Ejemplo:
'          ca | sa | do
'
'   2) ACENTOS (MODIFICADORES SILÁBICOS)
'
'      Acento tónico (sílaba principal):
'          ( ... )
'
'      Acento secundario:
'          [ ... ]
...
'          [pre] | (sen) | ta
'
'   3) MODIFICADORES FONÉMICOS (AFECTAN A UN SOLO FONEMA)
'
'      Nasalización:
'          *
'
...
'          :
'
'      Ligadura (africadas, unión de dos fonemas):
' _
'
'      Pausa breve:
'          .
...
Public Function EntradaMotor_ES(texto As String) As String
    ...
    Call SilabearAuto
    Call DetectarAcentos
    Call MarcarSilabasConAcentos

    Call RevisionSilabeo          ' aquí el usuario ve ( ) y [ ]
                                  ' y el resultado se guarda en SilabasFinal

    Call LimpiarMarcadores        ' ahora sí, sobre SilabasFinal
    Call ConvertirSilabasAFonemas ' usando SilabasFinal limpia
...
'
'    Call DetectarAcentos 'DetectarTonica
...
Private Sub DetectarAcentos()
    ...
    Dim i As Long
    Dim s As String
    Dim idxT As Long, idxS As Long
    Dim offset As Long
    Dim arrT() As String
    Dim arrS() As String
    Dim cT As Long, cS As Long
    ...
    For i = 1 To palabras.Count
        ...
        Call DetectarAcentosDeUnaPalabra_DesdeSilabas( _
                palabras(i), idxT, idxS)

        If idxT > 0 Then
            cT = cT + 1
            ReDim Preserve arrT(0 To cT - 1)
            arrT(cT - 1) = CStr(offset + idxT)
        End If

        If idxS > 0 Then
            cS = cS + 1
            ReDim Preserve arrS(0 To cS - 1)
            arrS(cS - 1) = CStr(offset + idxS)
        End If
...
Private Sub DetectarAcentosDeUnaPalabra_DesdeSilabas( _
        colSilabas As Collection, _
        ByRef idxT As Long, _
        ByRef idxS As Long)
...
Private Sub MarcarSilabasConAcentos()
    ...
    If objDTO.SilabaTonica <> "" Then idxT = Split(objDTO.SilabaTonica, ",")
    If objDTO.SilabaSecundaria <> "" Then idxS = Split(objDTO.SilabaSecundaria, ",")

    For i = LBound(partes) To UBound(partes)

        If EstaEnArray(CStr(i + 1), idxT) Then
            partes(i) = "(" & partes(i) & ")"
        ElseIf EstaEnArray(CStr(i + 1), idxS) Then
            partes(i) = "[" & partes(i) & "]"
        End If

    Next i

    objDTO.SilabasAuto = Join(partes, " | ")

End Sub

Private Function EstaEnArray(valor As String, arr() As String) As Boolean
    Dim i As Long
    On Error GoTo noArr
...
Private Sub LimpiarMarcadores()

    Dim partes() As String
    Dim i As Long

    partes = Split(objDTO.SilabasFinal, " | ")

    For i = LBound(partes) To UBound(partes)
        partes(i) = Replace(partes(i), "(", "")
        partes(i) = Replace(partes(i), ")", "")
        partes(i) = Replace(partes(i), "[", "")
        partes(i) = Replace(partes(i), "]", "")
    Next i

    objDTO.SilabasFinal = Join(partes, " | ")
...
'======================================================================================
'  5- CONVERSOR FONETICO
'======================================================================================

' ============================================================
'   5.1- MOTOR CONVERSOR FONETICO
' ============================================================
' ============================================================
...
'    EsSilabaMarcada = (cnt = 0)
    ...
    If InStr(contenido, "*") > 0 Then Exit Function
...
'Private Function Conv_Vocales(g As String, siguiente As String) As String
'
'    g = LCase$(g)
'    siguiente = LCase$(siguiente)
'
...
'    ' i/u + a/e/o
'    If (g = "i" Or g = "u") And (siguiente = "a" Or siguiente = "e" Or siguiente = "o") Then
'        Conv_Vocales = "14," & VocalID(siguiente)
'        Exit Function
'    End If
'
'    ' ============================
...
'
'    ' ============================================
'    '   1. Obtener las sílabas desde el DTO
'    '      Ejemplo: "ca-sa" ? ["ca","sa"]
'    ' ============================================
'    listaSilabas = Split(objDTO.SilabasFinal, " | ")
'
'    ' ============================================
'    '   2. Procesar cada sílaba
'    ' ============================================
'    For Each silaba In listaSilabas
'
'        'Set col = Conv_Silaba(silaba)
'        Temp = Conv_Silaba(CStr(silaba))
'
'        salida = salida & ", " & Temp
'        ' Añadir fonemas de la sílaba al resultado
''        For Each f In col
''            If resultado = "" Then
''                resultado = CStr(f)
''            Else
''                resultado = resultado & "," & CStr(f)
''            End If
''        Next f
'
'    Next silaba
'
'    ' ============================================
'    '   3. Guardar resultado final en el DTO
'    ' ============================================
'    objDTO.TextoFinal = resultado
'    objDTO.TextoFinal = Trim(Mid(salida, 2))
'
'End Sub
