...
Public Sub ConstruirCadenaFonemas_EU()
    ...
    frase = NormalizarVocales_EU(ObjDTO.SilabasFinal)
...
' ============================================================
Private Sub ProcesarSilaba_EU(ByVal silabaCruda As String)
    ...
    Do While i <= Len(silaba)
        ...
        grafema = DetectarDigrafo_EU(silaba, i)
        ...
        id = AsignarFonemaBase_EU(grafema, sigCh, antCh)
...
' ============================================================
Private Function DetectarDigrafo_EU(t As String, pos As Long) As String
    ...
    If pos + 1 <= L Then
        ...
        Select Case par
            ...
            Case "tx": DetectarDigrafo_EU = par: Exit Function

            ' Africada alveolar /ts/
            Case "tz", "ts": DetectarDigrafo_EU = par: Exit Function

            ' Vibrante múltiple (ya la trataremos como "rr")
            Case "rr": DetectarDigrafo_EU = par: Exit Function

            ' Geminadas ocasionales (préstamos)
            Case "dd", "tt": DetectarDigrafo_EU = par: Exit Function

            ' Palatal lateral /?/ (préstamos)
            Case "ll": DetectarDigrafo_EU = par: Exit Function

            ' Palatal nasal /?/ (préstamos)
            Case "ñ": DetectarDigrafo_EU = par: Exit Function

        End Select
    End If

    ' 2. Si no es dígrafo ? 1 letra
    DetectarDigrafo_EU = Mid$(t, pos, 1)
...
'   FONEMA BASE — EUSKERA
' ============================================================
Private Function AsignarFonemaBase_EU(grafema As String, _
    ...
    ' VOCALes
    Select Case grafema
        Case "a": AsignarFonemaBase_EU = 1: Exit Function   ' /a/
        Case "e": AsignarFonemaBase_EU = 2: Exit Function   ' /e/
        Case "i": AsignarFonemaBase_EU = 4: Exit Function   ' /i/
        Case "o": AsignarFonemaBase_EU = 5: Exit Function   ' /o/
        Case "u": AsignarFonemaBase_EU = 7: Exit Function   ' /u/
    End Select

    ' SEMIVOCALES
    If grafema = "i" And sig Like "[aeou]" Then
        AsignarFonemaBase_EU = 21: Exit Function   ' /j/
    End If

    If grafema = "u" And sig Like "[aeio]" Then
        AsignarFonemaBase_EU = 22: Exit Function   ' /w/
    End If

    ' NASALES
    Select Case grafema
        Case "m": AsignarFonemaBase_EU = 36: Exit Function
        Case "n": AsignarFonemaBase_EU = 37: Exit Function
        Case "ñ": AsignarFonemaBase_EU = 38: Exit Function
    End Select

    ' LATERALES
    If grafema = "l" Then AsignarFonemaBase_EU = 62: Exit Function
    If grafema = "ll" Then AsignarFonemaBase_EU = 63: Exit Function

    ' VIBRANTES
    If grafema = "rr" Then AsignarFonemaBase_EU = 60: Exit Function

    If grafema = "r" Then
        If ant = "" Or Not ant Like "[aeiou]" Then
            AsignarFonemaBase_EU = 60   ' inicial / postconsonántica ? múltiple
        Else
            AsignarFonemaBase_EU = 59   ' intervocálica ? simple
        End If
        Exit Function
    End If

    ' AFRICADAS
    If grafema = "tx" Then AsignarFonemaBase_EU = 57: Exit Function   ' /t?/

'    If grafema = "tz" Or grafema = "ts" Then
'        ' Puedes ajustar el ID si defines uno específico para /ts/
'        AsignarFonemaBase_EU = 46      ' /ts/ aproximado
'        Exit Function
'    End If

    If grafema = "ts" Then
        AsignarFonemaBase_EU = 72
        Exit Function
    End If

    If grafema = "tz" Then
        AsignarFonemaBase_EU = 73
        Exit Function
    End If

    ' FRICATIVAS
    If grafema = "s" Then AsignarFonemaBase_EU = 42: Exit Function   ' /s/
    If grafema = "z" Then AsignarFonemaBase_EU = 43: Exit Function   ' /z/
    If grafema = "x" Then AsignarFonemaBase_EU = 44: Exit Function   ' /?/
    If grafema = "j" Then AsignarFonemaBase_EU = 48: Exit Function   ' /x/
    If grafema = "h" Then AsignarFonemaBase_EU = 49: Exit Function   ' /h/ (o muda si luego la filtras)
    If grafema = "f" Then AsignarFonemaBase_EU = 40: Exit Function


    ' g + e/i ? fricativa velar sorda (como en ES)
    If grafema = "g" And (sig = "e" Or sig = "i") Then
        AsignarFonemaBase_EU = 48      ' /x/
        Exit Function
    End If

    ' OCLUSIVAS
    Select Case grafema
        Case "p": AsignarFonemaBase_EU = 30: Exit Function
        Case "b": AsignarFonemaBase_EU = 31: Exit Function
        Case "t", "tt": AsignarFonemaBase_EU = 32: Exit Function
        Case "d", "dd": AsignarFonemaBase_EU = 33: Exit Function
        Case "k": AsignarFonemaBase_EU = 34: Exit Function
        Case "g": AsignarFonemaBase_EU = 35: Exit Function
    End Select

    ' Detectar ligadura manual
    If grafema = "_" Then AsignarFonemaBase_EU = 84: Exit Function
    
    ' No reconocido ? 255
    AsignarFonemaBase_EU = 255
...
' ============================================================
Private Function NormalizarVocales_EU(ByVal texto As String) As String
    ' No tocamos grafías aquí; cualquier cosa rara ? ID 255 en el mapeo.
    NormalizarVocales_EU = texto
End Function
