...
Public Sub ConstruirCadenaFonemas_GL()
    ...
    frase = NormalizarVocales_GL(ObjDTO.SilabasFinal)
    ...
    For i = 0 To UBound(arrSilabas)
        ...
        ProcesarSilaba_GL silaba

        ' 5. Separador silábico
        ObjDTO.IdsFonemas = ObjDTO.IdsFonemas & "|"

SiguienteIteracion_GL:
    Next i
...
'---------------------------------------------------------
Private Sub ProcesarSilaba_GL(ByVal silabaCruda As String)
    ...
    Do While i <= Len(silaba)
        ...
        '-----------------------------------------
        grafema = DetectarDigrafoGL(silaba, i)
        ...
        '-----------------------------------------
        id = AsignarFonemaBaseGL(grafema, sigCh, antCh)
...
'    Do While i <= Len(silaba)
'
'        grafema = DetectarDigrafoEs(silaba, i)   ' Podemos dejar ES de momento
'
'        If i > 1 Then
'            antCh = Mid$(silaba, i - 1, 1)
'        Else
...
'        End If
'
'        ' De momento, reutilizamos el asignador ES:
...
Function NormalizarGrafemaGL(ByVal texto As String) As String
...
'
'    ' ============================
'    '   Mantener dígrafos gallegos
'    '   (no se tocan)
'    '   ch, ll, nh, rr, gu, qu, gü
'    ' ============================
'
'    NormalizarGrafemaGL = texto
'
...
Function DetectarDigrafoGL(texto As String, pos As Long) As String
    ...
    If pos + 1 > Len(texto) Then
        DetectarDigrafoGL = Mid$(texto, pos, 1)
        Exit Function
    End If
...
'===========================================================
Function AsignarFonemaBaseGL(grafema As String, _
...
'    ant = LCase$(ant)
    ...
    ' VOCALES
    Select Case grafema
        Case "a", "á": AsignarFonemaBaseGL = 1: Exit Function ' A / Á
        Case "e", "é": AsignarFonemaBaseGL = 2: Exit Function ' E cerrada
        Case "è", "ê": AsignarFonemaBaseGL = 3: Exit Function ' E aberta (dialectal)
        Case "i", "í": AsignarFonemaBaseGL = 4: Exit Function ' I / Í
        Case "o", "ó": AsignarFonemaBaseGL = 5: Exit Function ' O cerrada
        Case "ò", "ô": AsignarFonemaBaseGL = 6: Exit Function ' O aberta (dialectal)
        Case "u", "ú": AsignarFonemaBaseGL = 7: Exit Function ' U / Ú
    End Select
    ...
    '===========================================================
    Select Case grafema
        Case "p": AsignarFonemaBaseGL = 30: Exit Function
        Case "b", "v": AsignarFonemaBaseGL = 31: Exit Function
        Case "t": AsignarFonemaBaseGL = 32: Exit Function
        Case "d": AsignarFonemaBaseGL = 33: Exit Function
    End Select

    '===========================================================
    ' K
    '===========================================================
    If grafema = "k" Then AsignarFonemaBaseGL = 34: Exit Function
    ...
    If grafema = "c" Then
        If sig = "e" Or sig = "i" Then
            ...
            If CFG.ModoSibilantes = 0 Or CFG.ModoSibilantes = 1 Then
                AsignarFonemaBaseGL = 46   ' /?/
            Else
                AsignarFonemaBaseGL = 42   ' /s/
            End If
        Else
            AsignarFonemaBaseGL = 34       ' /k/
        End If
        Exit Function
    End If
    ...
    ' QU
    '===========================================================
    If grafema = "qu" Then AsignarFonemaBaseGL = 34: Exit Function
    ...
    If grafema = "g" Then
        If sig = "e" Or sig = "i" Then
            AsignarFonemaBaseGL = 48   ' /x/ (provisional para /?/)
        Else
            AsignarFonemaBaseGL = 35   ' /g/
        End If
        Exit Function
    End If
    ...
    ' GU
    '===========================================================
    If grafema = "gu" Then AsignarFonemaBaseGL = 35: Exit Function

    '===========================================================
    ' FRICATIVAS
    '===========================================================
    Select Case grafema
        Case "f": AsignarFonemaBaseGL = 40: Exit Function
'        Case "x": AsignarFonemaBaseGL = 48: Exit Function   ' /?/ gallego
        Case "x": AsignarFonemaBaseGL = 44: Exit Function   ' /?/ gallego
        Case "s": AsignarFonemaBaseGL = 42: Exit Function
        Case "z":
            If CFG.ModoSibilantes = 0 Or CFG.ModoSibilantes = 1 Then
                AsignarFonemaBaseGL = 46   ' /?/
            Else
    ...
    '===========================================================
    Select Case grafema
        Case "nh": AsignarFonemaBaseGL = 38: Exit Function   ' /?/
        Case "ñ": AsignarFonemaBaseGL = 38: Exit Function
        Case "m": AsignarFonemaBaseGL = 36: Exit Function
        Case "n": AsignarFonemaBaseGL = 37: Exit Function
    End Select

    '===========================================================
    ' LATERALES
    '===========================================================
    If grafema = "ll" Then AsignarFonemaBaseGL = 63: Exit Function
    If grafema = "lh" Then AsignarFonemaBaseGL = 63: Exit Function
    If grafema = "l" Then AsignarFonemaBaseGL = 62: Exit Function
    
    '===========================================================
    ' VIBRANTES
    '===========================================================
    If grafema = "rr" Then AsignarFonemaBaseGL = 60: Exit Function

    If grafema = "r" Then
        If ant = "" Or Not (ant Like "[aeiou]") Then
            AsignarFonemaBaseGL = 60   ' inicial ? vibrante múltiple
        Else
            AsignarFonemaBaseGL = 59   ' intervocálica ? simple
        End If
        Exit Function
    End If
    ...
    ' AFRICADA
    '===========================================================
    If grafema = "ch" Then AsignarFonemaBaseGL = 57: Exit Function

    '===========================================================
    ' H MUDA
    '===========================================================
    If grafema = "h" Then AsignarFonemaBaseGL = 0: Exit Function

    '===========================================================
    ' Si no se reconoce
    '===========================================================
    AsignarFonemaBaseGL = 255
...
Private Function NormalizarVocales_GL(ByVal texto As String) As String
