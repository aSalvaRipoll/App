Attribute VB_Name = "bas_Motor_GL_Main"
...
' ============================================================
'   ENTRADA PRINCIPAL DEL MOTOR GALLEGO
' ============================================================
' ----------------------------------------------------------------
...
Public Function Entrada_Motor_GL(texto As String) As String

    Set ObjDTO = New clsDTO_Motor

    ' 1) Texto + normalización
    ...
    ' 6) Generar fonética (cuando tengas el motor GL)
    Call ConstruirCadenaFonemas_GL
    
'    ' Debug opcional
'    Call MF_DebugDTO("Silabear")
    
    ' 7) Devolver resultado
    Entrada_Motor_GL = ObjDTO.SilabasAuto
...
Private Function AtaqueSilabicoValido(ByVal a As String, ByVal b As String) As Boolean
    ...
    Select Case grupo
        ...
        ' "tl" en gallego es muy marginal; si quieres, lo dejamos fuera:
        'Case "tl"
        '    AtaqueSilabicoValido = True
...
Private Function EsVocal(ByVal c As String) As Boolean
    EsVocal = InStr("aeiouáéíóú", LCase$(c)) > 0
End Function

Private Function EsVocalFuerte(ByVal c As String) As Boolean
    EsVocalFuerte = InStr("aáeéoó", LCase$(c)) > 0
End Function
...
'    ' Palabras de 6+ sílabas ? segunda secundaria
'    If n >= 6 Then
''ToDo: Hacer esta revisión en todos los motores
'        If pos2 > 2 Then
'            pos2 = tPos - 2    ' dos antes de la tónica
'        End If
'        If pos2 > 1 Then
