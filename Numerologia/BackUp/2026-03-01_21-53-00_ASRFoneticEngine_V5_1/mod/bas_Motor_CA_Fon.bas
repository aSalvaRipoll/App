...
Private Function DetectarDigrafo(t As String, pos As Long) As String
    ...
    If pos + 1 <= L Then
        ...
        ' GE / GI ? grafema complet (no dígraf, però necessari per fonètica)
        If par = "ge" Or par = "gi" Then
            DetectarDigrafo = par
            Exit Function
        End If

            ' CE / CI no són dígrafs ? es processen com C + vocal
        If par = "ce" Or par = "ci" Then
            'DetectarDigrafo = "c"
        ...
        Select Case par
            ...
            ' GÜE / GÜI (la U es pronuncia)
            Case "gü"
                If sig = "e" Or sig = "i" Then
                    DetectarDigrafo = "gü" & sig
                    Exit Function
                End If
...
'            Case "gü"
'                If sig = "e" Or sig = "i" Then
'                    DetectarDigrafo = "gü" & sig   ' g + w + vocal
'                    Exit Function
'                End If
'
'        End Select
...
''            Case "gü"
''                If sig = "e" Or sig = "i" Then
''                    DetectarDigrafo = "gü" & sig   ' devolver "güe" o "güi"
''                    Exit Function
''                End If
'
''                If sig = "e" Or sig = "i" Then
...
Private Function AsignarFonemaBase(grafema As String, _
    ...
    ' U muda en "gue/gui"
    If grafema = "u" And ant = "g" And (sig = "e" Or sig = "i") Then
        AsignarFonemaBase = 0   ' no genera fonema
        Exit Function
    End If
    ...
    Select Case grafema
        ...
        Case "u", "ú": AsignarFonemaBase = 7: Exit Function
        Case "ü": AsignarFonemaBase = 22: Exit Function
    End Select
    ...
    If grafema = "w" Or grafema = "ü" Then
    ...
    ' VIBRANTS
    If grafema = "rr" Then
        AsignarFonemaBase = 60: Exit Function
    End If
    ...
    If grafema = "ç" Then AsignarFonemaBase = 42: Exit Function ' /s/
    If grafema = "s" Then AsignarFonemaBase = 42: Exit Function
    If grafema = "z" Then AsignarFonemaBase = 43: Exit Function
    ' X ? /ks/ davant de i en paraules cultes (taxi, toxic, axioma…)
    If grafema = "x" Then
        If sig = "i" Then
            ObjDTO.IdsFonemas = ObjDTO.IdsFonemas & "34,42," ' /ks/ (o el codi que tinguis per /k/+/s/)
            AsignarFonemaBase = 0
            Exit Function
        End If
...
'    If grafema = "x" Then AsignarFonemaBase = 44: Exit Function
    ...
    'If grafema = "ce" Or grafema = "ci" Then AsignarFonemaBase = 42: Exit Function ' /s/
    If grafema = "c" And (sig = "e" Or sig = "i") Then AsignarFonemaBase = 42: Exit Function ' /s/
